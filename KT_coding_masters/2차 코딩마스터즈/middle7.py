...
def card_game_custom(n, cards):
    ...
    while idx < total_cards:
        ...
        max_diff = float('-inf')
        best_k = 1  # 철수가 가져갈 카드 수
        for k in range(1, 4):  # 철수는 1~3장의 카드를 가져갈 수 있음
            if idx + k > total_cards:
                break
            # 철수가 k장을 가져갔을 때 점수
            chulsoo_take = sum(cards[idx:idx + k])
            ...
            if idx + k < total_cards:
                ...
                younghee_take = 0
            # 점수 차이 계산
            diff = chulsoo_take - younghee_take
            if diff > max_diff:
                max_diff = diff
                best_k = k
...
result1 = card_game_custom(n, cards)
result2 = card_game_with_younghee_first(n,cards)
